...
function App() {
  ...
  const fetchLeadDetails = async (leadId) => {
    ...
    try {
      ...
      // Fetch signals
      const signalsResponse = await fetch(`${API_URL}/leads/${leadId}/signals`).catch(() => null);
      const signalsData = signalsResponse ? await signalsResponse.json() : { signals: [] };
      
      // Fetch outreach
      const outreachResponse = await fetch(`${API_URL}/leads/${leadId}/outreach`).catch(() => null);
      const outreachData = outreachResponse ? await outreachResponse.json() : { messages: [] };
      
      setSelectedLead({
        ...data,
        signals: signalsData.signals || [],
        outreach: outreachData.messages || []
      });
    } catch (error) {
      console.error('Failed to fetch lead details:', error);
    }
  ...
  const handleFullAnalysis = async (leadId) => {
    if (!window.confirm('Run full analysis? This will:\n• Research the company and contact\n• Detect buying signals\n• Generate personalized outreach\n\nThis may take 30-60 seconds.')) {
      return;
    }
    ...
    try {
      const response = await fetch(`${API_URL}/leads/${leadId}/full-analysis`, {
        method: 'POST'
      });
      ...
      if (result.status === 'success') {
        alert(`✅ Full analysis complete!\n\n` +
              `✓ Research completed\n` +
              `✓ ${result.signals_detected} signals detected\n` +
              `✓ Outreach generated (${result.personalization_score}/10)`);
      } else {
        alert(`⚠️ Analysis partially complete\n\n${result.errors.join('\n')}`);
      }
    } catch (error) {
            ...
            <div>
              <div className="content-header">
                ...
                <div className="content-meta">
                  ...
                  {selectedLead.lead.status === 'new' && (
                    <button
                      className="btn-primary"
                      onClick={() => handleFullAnalysis(selectedLead.lead.id)}
                      disabled={analyzing}
                    >
                      {analyzing ? (
                        <>
              ...
              <div className="content-grid">
                ...
                {selectedLead.signals && selectedLead.signals.length > 0 && (
                  <div className="content-section">
                    <h3 className="section-title">Buying Signals ({selectedLead.signals.length})</h3>
                        ...
                        <div key={idx} className="signal-card">
                          <div className="signal-header">
                            ...
                            <div 
                              ...
                              style={{ 
                                backgroundColor: `${getUrgencyColor(signal.urgency_score)}20`, 
                                color: getUrgencyColor(signal.urgency_score),
                                padding: '4px 12px',
                                borderRadius: '6px',
                                fontSize: '12px',
                                fontWeight: '700'
                ...
                {selectedLead.outreach && selectedLead.outreach.length > 0 && (
                  <div className="content-section">
                    <h3 className="section-title">Generated Outreach</h3>
                    {selectedLead.outreach.map((msg, idx) => (
                      <div key={idx} className="outreach-card">
                        ...
                        <div className="outreach-meta">
                          <span style={{textTransform: 'capitalize'}}>{msg.status}</span> • {new Date(msg.created_at).toLocaleDateString()}
                        </div>
                      </div>
                    ))}
                  </div>
